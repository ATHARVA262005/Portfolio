...
const Project = () => {
  ...
  const handleVideoPlay = (index) => {
    if (PROJECTS[index].previewVideo) {
  ...
  const closeButtonColor = activeVideo !== null && activeVideo >= 0 ? 
    (PROJECTS[activeVideo].bgColor === 'black' ? 'text-white' : 'text-black') 
    : 'text-black';
  ...
  {PROJECTS.map((project, index) => (
    <div key={index} className="mb-8 flex flex-wrap lg:justify-center">
      ...
      >
        ...
        >
          <GoPlay className='text-4xl text-slate-600 md:text-white' />
        </div>
      </motion.div>
      <motion.div
        whileInView={{ opacity: 1, x: 0 }}
  ...
  {activeVideo !== null && (
    ...
    >
      <div className="relative">
        ...
        >
          <source src={PROJECTS[activeVideo].previewVideo} type="video/mp4" />
        </video>
        <button 
